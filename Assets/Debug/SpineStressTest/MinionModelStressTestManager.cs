...
namespace DChildDebug.Spine.Tests
{
    ...
    public class MinionModelStressTestManager : MonoBehaviour
    {
        [SerializeField]
        ...
        private IEnumerator MinionModelStressTestRoutine(GameObject minionModel, string minionName)
        {
            if (minionModel == null)
            {
                Debug.LogError($"{minionName} is missing!");
            }
            else
            {
                ...
                {
                    yield return InstantiateInstances(minionModel, i, endofFrame);
        ...
        private IEnumerator InstantiateInstances(GameObject minionModel, int startingIndex, WaitForEndOfFrame endOfFrame)
        {
            for (int k = 0; k < m_instancesPerMonitoring; k++)
            {
                m_instantiator.InstantiateModel(minionModel, startingIndex + k);
        ...
        private IEnumerator TestRoutine()
        {
            ...
            for (int i = 0; i < m_minionPrefabs.Count; i++)
            {
                ...
                yield return MinionModelStressTestRoutine(minion, minion.name);
            }
            GenerateLogToFile();
        }
