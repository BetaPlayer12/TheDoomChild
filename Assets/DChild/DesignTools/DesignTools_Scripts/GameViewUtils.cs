...
public static class GameViewUtils
{
    ...
    public static void RemoveCustomSize(GameViewSizeGroupType sizeGroupType, string text)
    {
        var sizeIndex = FindSize(sizeGroupType, text);
        if (sizeIndex == -1)
            return;

        var group = GetGroup(sizeGroupType);
        object[] parameters = new object[] { sizeIndex };

        getGroup.ReturnType.GetMethod("RemoveCustomSize").Invoke(group, parameters);
    }
