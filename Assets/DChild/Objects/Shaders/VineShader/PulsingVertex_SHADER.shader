...
Shader "Gian/Pulsing Vertex"
{
    Properties
    {
	    ...
	    _ShakeDisplacement ("Displacement", Range (0, 5.0)) = 0
	    _ShakeTime ("Shake Time", Range (0, 5.0)) = 2
	    _ShakeWindspeed ("Shake Windspeed", Range (0, 5.0)) = 0.05
	    _ShakeBending ("Shake Bending", Range (0, 5.0)) = 0.1

	    //GrassWave Custom Properties
		_Freq("Frequency", Range(0,5)) = 0.3
		_Speed("Speed", Range(0,100)) = 100
		_Amp("Amplitude", Range(0,1)) = 0.03
    }
    ...
    SubShader
    {
        Tags
        {
            "Queue"="Transparent" //Original
            //"Queue"="AlphaTest" //GrassWave
        ...
        struct Input
        {
            float2 uv_MainTex;
            fixed4 color;
    		float2 uv_Illum; //GrassWave
			float3 vertColor; //GrassWave Custom
        };
        ...
        void vert (inout appdata_full v, out Input o)
        {
		    ...
		    //Custom GrassWave Codes END
        }

        void surf (Input IN, inout SurfaceOutput o)
        {
            fixed4 c = SampleSpriteTexture (IN.uv_MainTex) * IN.color; //Original
            o.Albedo = c.rgb * c.a; //Original
    		//fixed4 c = tex2D(_MainTex, IN.uv_MainTex) * _Color; //GrassWaves
    		//o.Albedo = c.rgb; //GrassWave
    		o.Emission = c.rgb * tex2D(_Illum, IN.uv_Illum).a; //GrassWave
            o.Alpha = c.a;
        }
        ENDCG
    }
