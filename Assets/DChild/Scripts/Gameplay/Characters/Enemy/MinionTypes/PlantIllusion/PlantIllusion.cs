...
{
    public class PlantIllusion : Minion, IFlinch, ITerrainPatroller
    {
        [SerializeField]
        ...
        private PlantIllusionAnimation m_animation;
        ...
        public void Attack(int attack)
        {
            if (Wait())
            {
                ...
                m_rootMotion.enabled = true;
                m_rootMotion.useY = false;
                m_behaviour.SetActiveBehaviour(StartCoroutine(AttackRoutine(attack)));
            }
        }
        #endregion
        ...
        protected IEnumerator DeathRoutine()
        {
            ...
            m_movement.Stop();
            m_animation.DoDeath();
            yield return new WaitForAnimationComplete(m_animation.animationState, PlantIllusionAnimation.ANIMATION_DEATH);
        ...
        private IEnumerator TurnRoutine()
        {
            m_waitForBehaviourEnd = true;
            m_animation.DoTurn();
            yield return new WaitForAnimationComplete(m_animation.animationState, PlantIllusionAnimation.ANIMATION_TURN);
            m_animation.DoIdle();
            yield return null;
            TurnCharacter();
            StopActiveBehaviour();
        ...
        private IEnumerator FlinchRoutine()
        {
            ...
            yield return new WaitForAnimationComplete(m_animation.animationState, PlantIllusionAnimation.ANIMATION_FLINCH);
            StopActiveBehaviour();
        }
        ...
        private IEnumerator AttackRoutine(int attack)
        {
            m_waitForBehaviourEnd = true;
            m_animation.DoSurprise();
            yield return new WaitForAnimationComplete(m_animation.animationState, PlantIllusionAnimation.ANIMATION_SURPRISE);
            if (attack == 1)
            {
            ...
            {
                m_animation.DoAttack1Hidden();
            }
            else if (attack == 4)
            {
                m_animation.DoAttack2Hidden();
            }
            yield return null;
            StopActiveBehaviour();
        }
        ...
        protected override void Awake()
        {
            ...
            m_animation = GetComponent<PlantIllusionAnimation>();
