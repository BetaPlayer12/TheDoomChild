...
{
    public class Mandragora : Minion, IFlinch, ITerrainPatroller
    {
        [SerializeField]
        ...
        [SerializeField]
        private BoxCollider2D m_aggroSensorCollider;
        [SerializeField]
        [TabGroup("Sensor Size")]
        private float m_sensorSize;


        [SerializeField]
        private AttackDamage m_damage;

        private MandragoraAnimation m_animation;
        ...
        public void SetState(bool isBurried, float sensorSize)
        {
            m_isBurried = isBurried;
            m_aggroSensorCollider.size = new Vector2(sensorSize, m_aggroSensorCollider.size.y);
        }
        ...
        public void Detect()
        {
            m_rootMotion.enabled = true;
            m_rootMotion.useY = false;
            m_animation.DoDetect(m_isBurried);
        }
        ...
        public void Attack()
        {
            if (Wait())
            {
                m_rootMotion.enabled = true;
                m_rootMotion.useY = false;
                m_animation.DoAttack(m_isBurried);
            }
        }
        #endregion
        ...
        public bool IsIdle()
        {
            if (m_animation.GetCurrentAnimation(0).ToString() == "Idle1" || m_animation.GetCurrentAnimation(0).ToString() == "Idle2")
            {
                return true;
            }
            else
        ...
        protected IEnumerator DeathRoutine()
        {
            ...
            yield return new WaitForAnimationComplete(m_animation.animationState, m_isBurried ? MandragoraAnimation.ANIMATION_DEATH1 : MandragoraAnimation.ANIMATION_DEATH2);
        ...
        private IEnumerator TurnRoutine()
        {
            m_waitForBehaviourEnd = true;
            m_animation.DoTurn();
            yield return new WaitForAnimationComplete(m_animation.animationState, MandragoraAnimation.ANIMATION_TURN);
            if (m_isBurried)
            {
                m_animation.DoIdle1();
            }
        ...
        private IEnumerator FlinchRoutine()
        {
            ...
            m_rootMotion.enabled = false;
            m_movement.Stop();
            m_animation.DoFlinch(m_isBurried);
            yield return new WaitForAnimationComplete(m_animation.animationState, MandragoraAnimation.ANIMATION_FLINCH);
            StopActiveBehaviour();
        }
        ...
        protected override void Awake()
        {
            ...
            m_animation = GetComponent<MandragoraAnimation>();
