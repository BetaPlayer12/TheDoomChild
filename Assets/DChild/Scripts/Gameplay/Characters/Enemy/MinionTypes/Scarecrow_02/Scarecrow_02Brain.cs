...
{
    public class Scarecrow_02Brain : MinionAIBrain<Scarecrow_02>, IAITargetingBrain
    {
        [SerializeField]
        ...
        private void Behaviors()
        {
            switch (m_behaviorCount)
            {
                case 1:
                    ...
                    {
                        if (GetDistance() < m_summonRange && m_behaviorChance < m_summonChance && GetDistance() > m_castRange)
                        {
                            m_minion.SummonCrows(m_target);
                            m_currentBehavior = 0;
                        }
                        else
                        {
