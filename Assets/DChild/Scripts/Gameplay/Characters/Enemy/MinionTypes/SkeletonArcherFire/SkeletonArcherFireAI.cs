...
{
    [AddComponentMenu("DChild/Gameplay/Enemies/Minion/SkeletonArcherFire")]
    public class SkeletonArcherFireAI : CombatAIBrain<SkeletonArcherFireAI.Info>, IBattleZoneAIBrain, IResetableAIBrain, IKnockbackable
    {
        [System.Serializable]
        public class Info : BaseInfo
        {
            ...
            [SerializeField]
            private SimpleAttackInfo m_jumpShootAttack = new SimpleAttackInfo();
            public SimpleAttackInfo jumpShootAttack => m_jumpShootAttack;
            ...
            public override void Initialize()
            {
                ...
                m_shootAttack.SetData(m_skeletonDataAsset);
                m_jumpShootAttack.SetData(m_skeletonDataAsset);
        ...
        private void UpdateAttackDeciderList()
        {
            m_attackDecider.SetList(new AttackInfo<Attack>(Attack.ShootAttack, m_info.shootAttack.range),
                                    new AttackInfo<Attack>(Attack.JumpShootAttack, m_info.jumpShootAttack.range));
            m_attackDecider.hasDecidedOnAttack = false;
        }
            ...
            if (m_targetInfo.isValid)
            {
                ...
                switch (m_chosenAttack)
                {
                    case Attack.ShootAttack:
                        ...
                        break;
                    case Attack.JumpShootAttack:
                        Vector2 spitPos = m_projectilePoint.transform.position;
                        Vector3 v_diff = (m_lastTargetPos - spitPos);
                        float atan2 = Mathf.Atan2(v_diff.y, v_diff.x);
                        var aimRotation = atan2 * Mathf.Rad2Deg;
                        aimRotation = aimRotation - 15f;

                        ProjectileLaunchHandle launchHandle = new ProjectileLaunchHandle();
                        for (int i = 0; i < 3; i++)
                        {
                            m_projectilePoint.transform.rotation = Quaternion.Euler(0f, 0f, aimRotation);
                            var spawnDirection = m_projectilePoint.transform.right;
                            //var instance = GameSystem.poolManager.GetPool<ProjectilePool>().GetOrCreateItem(m_info.projectile.projectileInfo.projectile);
                            //instance.transform.position = m_projectilePoint.transform.position;
                            //var component = instance.GetComponent<Projectile>();
                            //component.ResetState();

                            launchHandle.Launch(m_info.projectile.projectileInfo.projectile, m_projectilePoint.transform.position, spawnDirection, m_info.projectile.projectileInfo.speed);
                            aimRotation += 15f;
                        }
                        break;
                }
        ...
        private void Update()
        {
            ...
            switch (m_stateHandle.currentState)
            {
                ...
                case State.Attacking:
                    ...
                    if (Vector2.Distance(transform.position, m_targetInfo.position) <= m_info.shootAttack.range)
                    {
                        m_animation.EnableRootMotion(true, false);
                        m_attackHandle.ExecuteAttack(m_info.shootAttack.animation, m_info.idleAnimation.animation);
                        m_chosenAttack = Attack.ShootAttack;
                    }
                    else
                    {
                        m_animation.EnableRootMotion(true, false);
                        m_attackHandle.ExecuteAttack(m_info.jumpShootAttack.animation, m_info.idleAnimation.animation);
                        m_chosenAttack = Attack.JumpShootAttack;
                    }
                ...
                case State.Chasing:
                    {
                        if (IsFacingTarget())
                        {
                            ...
                            if (m_attackDecider.hasDecidedOnAttack && IsTargetInRange(m_info.jumpShootAttack.range) && !m_wallSensor.allRaysDetecting && !ShotBlocked())
                            {
                                m_movement.Stop();
                                m_animation.SetAnimation(0, m_info.idleAnimation, true);
