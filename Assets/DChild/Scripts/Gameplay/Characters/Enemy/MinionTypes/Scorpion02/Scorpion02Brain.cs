...
{
    public class Scorpion02Brain : MinionAIBrain<Scorpion02>, IAITargetingBrain
    {
        [SerializeField]
        ...
        private void Behaviors()
        {
            switch (m_behaviorCount)
            {
                case 1:
                    ...
                    {
                        if (GetDistance() < m_attackRange && m_behaviorChance < m_attackChance && GetDistance() > m_idleRange -5)
                        {
                            m_minion.Attack();
                            m_currentBehavior = 0;
                        }
