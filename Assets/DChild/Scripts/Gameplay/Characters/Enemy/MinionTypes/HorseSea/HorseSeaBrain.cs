...
{
    public class HorseSeaBrain : MinionAIBrain<HorseSea>, IAITargetingBrain
    {
        [SerializeField]
        ...
        private void Behaviors()
        {
            switch (m_behaviorCount)
            {
                case 1:
                    ...
                    {
                        if (GetDistance() < m_Attack1Range && m_behaviorChance < m_Attack2Chance && GetDistance() > m_castRange)
                        {
                            m_minion.Attack1();
                            m_currentBehavior = 0;
                        }
