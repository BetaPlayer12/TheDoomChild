...
{
    [AddComponentMenu("DChild/Gameplay/Enemies/Minion/DummyTest")]
    public class DummyTestAI : CombatAIBrain<DummyTestAI.Info>, IResetableAIBrain
    {
        [System.Serializable]
        public class Info : BaseInfo
        {
            ...
            [SerializeField, ValueDropdown("GetAnimations")]
            private string m_death1Animation;
            public string death1Animation => m_death1Animation;
            [SerializeField, ValueDropdown("GetAnimations")]
            private string m_death2Animation;
            public string death2Animation => m_death2Animation;
            [SerializeField, ValueDropdown("GetAnimations")]
            private string m_death3Animation;
            public string death3Animation => m_death3Animation;
            [SerializeField, ValueDropdown("GetAnimations")]
        ...
        private FlinchHandler m_chosenFlinchhandle;
        ...
        private string DynamicDeathAnimation()
        {
            var deathIndex = UnityEngine.Random.Range(0, 3);
            switch (deathIndex)
            {
                case 0:
                    return m_info.death1Animation;
                case 1:
                    return m_info.death2Animation;
                case 2:
                    return m_info.death3Animation;
            }
            return null;
        }
        ...
        protected override void OnDestroyed(object sender, EventActionArgs eventArgs)
        {
            ...
            m_animation.SetAnimation(0, DynamicDeathAnimation(), false);
        ...
        private FlinchHandler DynamicFlinchHandle()
        {
            var flinchIndex = UnityEngine.Random.Range(0, m_flinchHandles.Count);
            return m_flinchHandles[flinchIndex];
        }
        ...
        private void OnFlinchStart(object sender, EventActionArgs eventArgs)
        {
            if (m_animation.GetCurrentAnimation(0).ToString() == m_info.idleAnimation)
            {
                StopAllCoroutines();
                m_selfCollider.enabled = false;
                m_chosenFlinchhandle = DynamicFlinchHandle();
                m_chosenFlinchhandle.m_autoFlinch = true;
        ...
        {
            if (m_chosenFlinchhandle.m_autoFlinch)
            {
                m_chosenFlinchhandle.m_autoFlinch = false;
                m_stateHandle.ApplyQueuedState();
            }
        }
        ...
        private IEnumerator RandomIdleRoutine()
        {
            while (true)
            {
                ...
                var currentTimer = 0f;
                var idleAnim = m_info.idleAnimation;
        ...
        protected override void Awake()
        {
            ...
            m_deathHandle.SetAnimation(DynamicDeathAnimation());
            for (int i = 0; i < m_flinchHandles.Count; i++)
            {
                m_flinchHandles[i].FlinchStart += OnFlinchStart;
