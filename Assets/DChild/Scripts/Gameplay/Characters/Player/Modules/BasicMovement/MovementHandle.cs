...
namespace DChild.Gameplay.Characters.Players.Behaviour
{
    public class MovementHandle : MonoBehaviour, IComplexCharacterModule
    {
        ...
        private RaySensor m_platformDecSlopeSensor;
        ...
        public void Initialize(ComplexCharacterInfo info)
        {
            ...
            m_platformSlopeSensor = info.GetSensor(PlayerSensorList.SensorType.Slope);
            m_platformDecSlopeSensor = info.GetSensor(PlayerSensorList.SensorType.DecSlope);
        ...
        private void SlopeMovement(Vector2 moveDirection)
        {
            ...
            if (m_platformSlopeSensor.isDetecting == true)
            {
                
                var hits = m_platformSlopeSensor.GetHits();
                slopeAngle = Vector2.Angle(hits[0].normal, Vector2.up);
                if(slopeAngle < 40.0f)
                {
                    m_endpos = new Vector3(m_modelObj.transform.rotation.x, m_modelObj.transform.rotation.y, slopeAngle * moveDirection.x);
                }
                else
                {
            ...
            {
                m_platformDecSlopeSensor.Cast();
                var hits = m_platformDecSlopeSensor.GetHits();
                slopeAngle = Vector2.Angle(hits[0].normal, Vector2.up);
                if(slopeAngle < 40.0f)
                {
                    m_endpos = new Vector3(m_modelObj.transform.rotation.x, m_modelObj.transform.rotation.y, slopeAngle * moveDirection.x);
