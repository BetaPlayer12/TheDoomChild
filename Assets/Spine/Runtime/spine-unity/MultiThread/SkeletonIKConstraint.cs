...
namespace DChild
{
    ...
    public struct SkeletonIKConstraint
    {
        public SkeletonIKConstraint(IkConstraint constraint)
        {
            ...
            if (boneCount > 0)
            {
                bone1 = new SkeletonBoneIndex(constraint.bones.Items[0]);
                if (boneCount == 2)
                {
                    bone2 = new SkeletonBoneIndex(constraint.bones.Items[1]);
                }
                else
                {
                    bone2 = new SkeletonBoneIndex(null);
                }
            }
            else
            {
                bone1 = new SkeletonBoneIndex(null);
                bone2 = new SkeletonBoneIndex(null);
            }
        ...
        public SkeletonBoneIndex bone1;
        public SkeletonBoneIndex bone2;
        ...
        public void Apply(SkeletonBone targetBone, ref SkeletonBone bone, SkeletonBone boneParent)
        {
            Apply(ref bone, targetBone.worldX, targetBone.worldY, compress, stretch, uniform, alpha, boneParent);
        }
                            ...
                            float alpha, SkeletonBone parent)
        {
                ...
                default:
                    {
                        ...
                        tx = (x * pd - y * pb) / d - bone.ax;
                        ty = (y * pa - x * pc) / d - bone.ay;
                        Debug.Log($"Default {bone.hashCode} == {x}-{y}-{pd}-{pa}-{pb}-{pc}-{d}-{bone.ax}-{bone.ay}");
                        break;
                    }
            }
            ...
            Debug.Log($"Bone {bone.hashCode} == {bone.arotation}- {rotationIK} -{alpha}");
        ...
        {
            var targetX = targetBone.worldX;
            var targetY = targetBone.worldY;
            if (alpha == 0)
            {
            ...
            if (l1 < 0.0001f)
            {
                Apply(ref parentBone, targetX, targetY, false, stretch, false, alpha, parentBoneParent);
            ...
            x = targetX - parentBoneParent.worldX;
            y = targetY - parentBoneParent.worldY;
